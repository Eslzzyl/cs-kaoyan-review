...
\begin{enumerate}
    \item {\bf 归零编码 RZ}。每个时钟周期的中间都要归零，提供了自同步机制，但归零要占用一点带宽。
    \item {\bf 非归零编码 NRZ}。不归零，但无法同步，需要额外的时钟线。
    \item {\bf 反向非归零编码 NRZI}。用信号的反转代表0，信号保持不变代表1，有同步且不会占用带宽。
...
\begin{itemize}
    \item \textbf{调制}是把信号注入载波的过程，和模数转换没有关系（下同）。
    \item \textbf{解调}是把信号从载波中分离的过程。
\end{itemize}

\begin{problem}
    利用模拟通信信道传输数字信号的方法称为\underline{频带传输}。
\end{problem}

\begin{solution}
    \begin{itemize}
        \item 将基带信号直接传送到通信链路（数字信道）上，叫\textbf{基带传输}。
        \item 将基带信号调制后送到通信链路（模拟信道）上，叫\textbf{频带传输}。
    \end{itemize}
\end{solution}
...
\begin{table}
    ...
    \resizebox{\linewidth}{!}{
        \begin{tabular}{|c|c|c|}
            \hline
            & \textbf{数据报服务} & \textbf{虚电路服务} \\ \hline
            连接的建立 & 不需要 & 必须有 \\ \hline
            目的地址 & 每个分组都有完整的目的地址 & \makecell{仅在建立连接阶段使用，\\ 之后每个分组使用虚电路号} \\ \hline
            路由选择 & 每个分组独立进行路由选择和转发 & 属于同一条虚电路的分组按照同一路由转发 \\ \hline
            分组顺序 & 不保证分组按顺序到达 & 保证分组按顺序到达 \\ \hline
            可靠性 & 不保证可靠通信 & 保证可靠通信 \\ \hline
            对网络故障的适应性 & \makecell{出故障的结点丢失分组，\\ 其他分组路径选择发生变化时可以正常传输} & 所有经过故障结点的虚电路都瘫痪 \\ \hline
            差错处理和流量控制 & 由用户主机进行流量控制 & 可由分组交换网络负责，也可由用户负责 \\
            \hline
        \end{tabular}
    }
\end{table}
...
\begin{solution}
    B。传统以太网采用广播的方式发送信息，同一时间只允许一台主机发送信息，否则各主机之间就会形成冲突，因此只能使用半双工方式。
\end{solution}
...
这$k$个检测位被安排在新的码字的第$2^{k−1}$位上，即第1, 2, 4, 8, 16...位。

然后，为每个检测位安排小组，其中第$i$个小组记作$g_i$，它是$C_{2^{i-1}}$这个检测位对应的小组。$g_i$满足：
\begin{itemize}
    \item $g_i$小组独占第$2^{i-1}$位。
    \item $g_i$和$g_j$小组共同占有第$2^{i-1}+2^{j-1}$位。
    \item $g_i$和$g_j$和$g_k$小组共同占有第$2^{i-1}+2^{j-1}+2^{k-1}$位。
    \item 依此类推。
\end{itemize}
...
\textcolor{red}{本章是历年考察的重中之重。}

\subsection{网络层的功能}

\subsubsection{异构网络互联}

异构网络指的是传输介质、数据编码方式、链路控制协议、数据单元格式和转发机制不同的网络，它们在物理层和数据链路层都有不同。
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item A类：10.0.0.0\~10.255.255.255，共1个网段
        \item B类：172.16.0.0\~172.31.255.255，共16个网段
        \item C类：192.168.0.0\~192.168.255.255，共256个网段
    \end{itemize}
...
\begin{table}
    ...
    \resizebox*{\linewidth}{!}{
        \begin{tabular}{|c|c|c|c|c|}
            \hline
            \textbf{协议} & \textbf{RIP} & \textbf{OSPF} & \multicolumn{2}{|c|}{\textbf{BGP}} \\ \hline
            类型 & 内部 & 内部 & \multicolumn{2}{|c|}{外部} \\ \hline
            路由算法 & 距离-向量 & 链路状态 & \multicolumn{2}{|c|}{路径-向量} \\ \hline
            承载协议 & UDP & IP & \multicolumn{2}{|c|}{TCP(:179)} \\ \hline
            所属层 & 应用层 & 网络层 & \multicolumn{2}{|c|}{应用层} \\ \hline
            路径选择 & 跳数最少 & 代价最低 & \multicolumn{2}{|c|}{较好，非最佳} \\ \hline
            交换结点 & 和本结点相邻的路由器 & 网络中所有路由器 & \multicolumn{2}{|c|}{和本结点相邻的路由器} \\ \hline
            \multirow{2}{*}{交换内容} & \multirow{2}{*}{自己的路由表} & \multirow{2}{*}{与自己相邻的所有路由器的链路状态} & 首次 & 整个路由表 \\ \cline{4-5}
            & & & 非首次 & 有变化的部分 \\
            \hline
        \end{tabular}
    }
...
可以使用单播来仿真组播，但会引起较大的处理开销和网络上太多的交通量。
...
IPv4的组播地址从D类地址中分配。D类地址范围是224.0.0.0\~ 239.255.255.255。每个D类地址标志一个组播组。并非所有的D类地址都可作为组播地址。
...
% 以下注释的部分，是计网期末复习的内容，王道书上没有相关的内容，可认为考研不涉及。
...
文件列表（LIST命令）是通过数据连接传送的。

匿名FTP访问通常使用anonymous作为用户名，密码可以是任意字符串。
...
\begin{table}[h]
    ...
    \resizebox{\linewidth}{!}{
        \begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}
            \hline
            应用程序 & \makecell{FTP\\ 数据连接} & \makecell{FTP\\ 控制连接} & TELNET & SMTP & DNS & TFTP & HTTP & POP3 & SNMP \\
            \hline
