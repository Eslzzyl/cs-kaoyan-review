...
\begin{enumerate}
  \item {\bf 字长}，指计算机进行一次整数运算所能处理的二进制数据的位数。
  \item {\bf 数据通路带宽}
  \item {\bf 主存容量}。MAR的位数反映了存储单元的个数，MDR的位数反映了存储单元的字长。
  \item {\bf 运算速度}
  \begin{enumerate}
...
\begin{enumerate}
  ...
  \begin{equation*}
    \text{Cache行号}=\text{主存块号}\ \text{mod}\  \text{Cache总行数}
  \end{equation*}
  ...
  主存中的任何一块都可以装入Cache的任何位置，每行的标记用于指出该行取自主存的哪一块。
  ...
  将Cache分成Q个大小相等的组，组间采用直接映射，组内采用全相联映射。Q=1时为全相联映射，Q=Cache行数时为直接映射。这是对上面两种方式的折中。

  假设每组有r个Cache行，则称为\textbf{r路组相联}。

  选定适当的路数，可使组相联的成本接近直接映射，性能接近全相联映射。
...
当Cache大小、主存块大小一定时，
\begin{itemize}
  \item 直接映射的命中率最低，全相联映射的命中率最高。
  \item 直接映射的判断开销最小、所需时间最短，全相联映射的判断开销最大、所需时间最长。
  \item 直接映射标记所占的额外空间开销最少，全相联映射标记所占的额外空间开销最大。
\end{itemize}
...
\begin{itemize}
  \item 写命中
  \begin{itemize}
    \item 全写法（写直达法、write-through）：写命中时，数据同时写入Cache和主存。
    \begin{itemize}
      \item 优点：简单，能随时保持主存数据的正确性。
      \item 缺点：增加了访存次数，降低了Cache的效率。
    \end{itemize}

    可在Cache和主存之间设置一个FIFO的写缓冲队列，以改善速度不匹配的问题。但写过于频繁时，可能会溢出。

    \item 回写法（写回法、write-back）：写命中时，仅仅把数据写入Cache，而不立即写入主存。仅当此块被换出时才写回主存。
    \begin{itemize}
      \item 优点：减少了访存次数。
      \item 缺点：存在不一致的隐患。需要为每个Cache行设置一个脏位。脏位为0，说明块未被修改过，替换时无需写回主存。
    \end{itemize}
  \end{itemize}
  \item 写不命中
  \begin{itemize}
    \item 写分配法（write-allocate）：加载主存中的块到Cache中，然后更新这个Cache块。
    \item 非写分配法（not-write-allocate）：只写入主存，不动Cache。
  \end{itemize}
...
\begin{enumerate}
  \item {\bf 单指令周期}：对所有指令都用相同的时间执行，指令是串行执行的。每条指令都用一个时钟周期执行完成，因此时钟周期取决于执行时间最长的指令。
  \item {\bf 多指令周期}：对不同指令使用不同数量的时钟周期执行，但指令仍然是串行执行的。
  \item {\bf 流水线方案}：略
\end{enumerate}
...
有以下几种：
\begin{enumerate}
  \item {\bf CPU内部单总线方式}，将所有寄存器的输入、输出端都连接到一条公共通路上，结构简单，但有很多冲突。
  \item {\bf CPU内部多总线方式}。
  \item {\bf 专用数据通路方式}。不用共享总线，而是根据数据通路设计特定的连线，性能很好，但硬件量很大。
\end{enumerate}
...
\begin{enumerate}
  ...
  \item {\bf DMA和中断方式的区别}
  \begin{itemize}
    \item 中断需要切换程序，意味着要保护和恢复现场；DMA方式不中断现有程序，除了预处理和后处理，其他时候不占用CPU。
    \item 中断请求的响应只能在指令周期结束后进行，DMA请求的响应可以在任意一个机器周期结束后进行。
    \item DMA速度快。
    \item DMA优先级高于中断请求。
    \item 中断方式能处理异常，DMA只能传送大批量数据。
    \item 中断方式靠程序传送，DMA方式靠硬件传送。
  \end{itemize}
\end{enumerate}
